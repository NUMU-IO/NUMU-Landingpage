/**
 * OriginKit — Dither Reveal (preset `base`).
 *
 * Retrieved through OriginKit MCP (`get_component dither-reveal`, stack
 * vite / tailwind / typescript) on 2026-08-23. VENDOR SOURCE — keep it close
 * to as-delivered so it can be re-fetched and diffed. Numueg's configuration
 * lives in `../DitherReveal.tsx`.
 *
 * Local deltas, and only these:
 *   • `"use client"` removed — Vite SPA, not Next.js.
 *   • Two palette uniforms added (`uInk`, `uPaper`). Upstream renders the
 *     dithered plate as pure greyscale (`vec3(tone)`); the identity lock has
 *     no greyscale, so the plate now interpolates between two Numueg colours
 *     supplied by the caller. Nothing else in the shader changed.
 *   • `onReady` / `onFail` callbacks so the wrapper can fall back to a plain
 *     `<img>` when WebGL is unavailable — the spec requires a static
 *     fallback and upstream just renders nothing.
 */

import { useEffect, useRef, type CSSProperties } from 'react';

const DITHER_INDEX: Record<string, number> = { bayer8: 0, lines: 1, noise: 2 };
...
export interface DitherRevealProps {
  ...
  waveSpeed?: number;
  waveDensity?: number;
  /** Local addition — dark end of the dithered plate, as [r,g,b] 0..1. */
  inkColor?: [number, number, number];
  /** Local addition — light end of the dithered plate, as [r,g,b] 0..1. */
  paperColor?: [number, number, number];
  /** Local addition — called when WebGL could not start. */
  onFail?: () => void;
  style?: CSSProperties;
}
...
function settings(p: DitherRevealProps) {
  ...
  const speed = waveOn ? clamp(p?.waveSpeed, 1, 100, DEFAULTS.waveSpeed) / 100 : 0;
  return {
...
export default function OriginKitDitherReveal(props: DitherRevealProps) {
  const {
    image,
    inkColor = [0, 0.121, 0.247],
    paperColor = [0.984, 0.964, 0.929],
    onFail,
    style,
  } = props;
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      try {
        ...
        /* tainted upload — keep the placeholder */
      }
    };
    img.onerror = () => onFail?.();
    img.src = image;
    ...
    function render() {
      ...
      gl!.uniform3f(uInk, inkColor[0], inkColor[1], inkColor[2]);
      gl!.uniform3f(uPaper, paperColor[0], paperColor[1], paperColor[2]);
...
// Numueg palette ends for the dithered plate.
uniform vec3 uInk;
uniform vec3 uPaper;
...
void main() {
    ...
    // Local change: the plate is a Numueg two-tone ramp, not greyscale.
    vec3 ditherColor = mix(uInk, uPaper, tone);
    ...
    gl_FragColor = vec4(mix(ditherColor, color.rgb, revealAmount), 1.0);
}
`;
